...
const RECTANGLEROI = "RectangleROI";
const EPSILON = 1e-4;

const trackingIdentifierTextValue = `${CORNERSTONE_3D_TAG}:${RECTANGLEROI}`;
...
class RectangleROI {
    ...
    static getTID300RepresentationArguments(tool, worldToImageCoords) {
        ...
        if (!referencedImageId) {
            throw new Error(
                "RectangleROI.getTID300RepresentationArguments: referencedImageId is not defined"
            );
        }
...
RectangleROI.isValidCornerstoneTrackingIdentifier = TrackingIdentifier => {
    ...
    // the RectangleRoi toolName (which was in the old cornerstone) to RectangleROI
    return toolType.toLowerCase() === RECTANGLEROI.toLowerCase();
};
